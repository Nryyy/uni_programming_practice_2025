...
namespace HromadaWEB.BL.Services
{
    public class AuthService : IAuthService
    {
        ...
        {
            var user = await _context.Users.FirstOrDefaultAsync(u => u.Email == request.Email); // Перевірка за email
            if (user is null)
            {
        ...
        public async Task<string?> RegisterAsync(RegistrationDto request)
        {
            ...
            if (user == null) return "Role 'PendingConfirmation' not found."; // Якщо роль не знайдена
        ...
        private string CreateToken(User user)
        {
            var claims = new Claim[]
            {
                ...
                // Якщо необхідно, додайте додаткові claim (наприклад, для ролей або інших атрибутів)
            };
